(() => {
  ...
  function getFilteredDoors() {
    const query = Dom.elements['search-input'].value.trim().toLocaleLowerCase('ru');
    if (!query) return doors;
    return doors.filter(door => Object.values(door).some(value =>
  ...
  async function deleteDoor(door) {
    const accepted = await Dom.confirm('Удалить дверь?', `${door.dk || 'Эта дверь'} будет удалена без возможности отмены.`);
    if (!accepted) return;
  ...
  function saveDoor(event) {
    ...
    const duplicate = doors.find(item => item.dk.toLocaleLowerCase('ru') === door.dk.toLocaleLowerCase('ru') && item.id !== door.id);
    if (duplicate) {
      alert(`Дверь ${door.dk} уже существует.`);
      return;
    }
    const index = doors.findIndex(item => item.id === door.id);
    if (index >= 0) doors[index] = door; else doors.push(door);
    Storage.save(doors);
  ...
  async function importCsv() {
    ...
    try {
      ...
      if (mode === 'replace') {
        ...
        const byDk = new Map(doors.map(door => [door.dk.toLocaleLowerCase('ru'), door]));
      ...
      Storage.save(doors);
      Dom.elements['import-status'].textContent = `Импортировано записей: ${imported.length}.`;
  ...
  function bindEvents() {
    document.querySelectorAll('.nav-button').forEach(button => button.addEventListener('click', () => Dom.showScreen(button.dataset.screen)));
    Dom.elements['search-input'].addEventListener('input', render);
    Dom.elements['add-door-button'].addEventListener('click', () => Dom.openDoorDialog());
    Dom.elements['door-form'].addEventListener('submit', saveDoor);
    Dom.elements['close-dialog-button'].addEventListener('click', () => Dom.elements['door-dialog'].close());
    Dom.elements['cancel-door-button'].addEventListener('click', () => Dom.elements['door-dialog'].close());

    Dom.elements['csv-file-input'].addEventListener('change', event => {
      selectedFile = event.target.files[0] || null;
      Dom.elements['import-button'].disabled = !selectedFile;
      Dom.elements['import-status'].textContent = selectedFile ? `Выбран файл: ${selectedFile.name}` : '';
    });
    Dom.elements['import-button'].addEventListener('click', importCsv);
    Dom.elements['export-button'].addEventListener('click', () => Csv.download(doors));
    Dom.elements['reset-button'].addEventListener('click', async () => {
      const accepted = await Dom.confirm('Восстановить исходные данные?', 'Все локальные изменения будут удалены.', 'Восстановить');
      if (!accepted) return;
      doors = Storage.reset();
      render();
    });
