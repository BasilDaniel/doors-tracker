(() => {
  ...
  function getFilteredDoors() {
    ...
    return doors.filter((door) => {
      ...
      // all означает отсутствие фильтра.
      // Пустая строка означает поле без значения.
  ...
  function render() {
    ...
    Dom.renderDoors(sorted, {
      onEdit: editDoor,
      onDelete: deleteDoor,
    });
  }

  // Открывает существующую дверь для редактирования.
  ...
  function saveDoor(event) {
    ...
    // Проверяет дубликаты только если
    // DK или designation заполнены.
    const duplicate = doors.find((item) => {
      ...
      // Полностью пустые по DK/designation записи
      // разрешены и считаются разными.
      return false;
    });
  ...
  async function importCsv() {
    ...
    try {
      ...
      if (mode === "replace") {
        ...
        // DK используется как основной ключ.
        // Если DK нет — используется designation.
        // Полностью безымянные записи остаются отдельными.
        const getKey = (door) => {
          if (door.dk) {
            return `dk:${door.dk.toLocaleLowerCase("ru")}`;
          }
  ...
  function registerServiceWorker() {
    ...
    navigator.serviceWorker
      ...
      .then((registration) => {
        // Проверяет обновление service worker
